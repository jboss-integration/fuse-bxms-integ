...
<xsl:template match="//*[local-name()='subsystem' and contains(namespace-uri(),'urn:jboss:domain:switchyard')]/*[local-name()='modules']">
    <xsl:choose>
      <xsl:when test="ns1:module[@identifier='org.switchyard.component.bpm'] and ns1:module[@identifier='org.switchyard.component.rules']">
         <xsl:copy>
		<xsl:apply-templates select="@*|node()"/>
        </xsl:copy> 
      </xsl:when>
       <xsl:when test="ns1:module[@identifier='org.switchyard.component.bpm']">
         <xsl:copy>
		<xsl:apply-templates select="@*|node()"/>
        </xsl:copy> 
      </xsl:when>
      <xsl:when test="ns1:module[@identifier='org.switchyard.component.rules']">
         <xsl:copy>
		<xsl:apply-templates select="@*|node()"/>
        </xsl:copy>      
