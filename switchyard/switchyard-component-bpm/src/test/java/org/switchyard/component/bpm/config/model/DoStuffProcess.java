...
@BPM(persistent = true, processId = "theProcessId", channels = @Channel(name = "theName", operation = "theOperation", reference = "theReference", value = BPMModelTests.TestChannel.class), listeners = @Listener(DebugProcessEventListener.class), loggers = @Logger(interval = 2000, log = "theLog", type = LoggerType.CONSOLE), manifest = @Manifest(
...
resources = {@Resource(location = "foobar.bpmn", type = "BPMN2"),
             @Resource(location = "foobar.xls", type = "DTABLE", detail = @ResourceDetail(inputType = "XLS", worksheetName = "MySheet")) // SWITCHYARD-1662,
                                                                                                                                         // usingExternalTypes=true))
}), properties = @Property(name = "foo", value = "bar"), userGroupCallback = @UserGroupCallback(value = BPMModelTests.TestUserGroupCallback.class, properties = @Property(name = "rab", value = "oof")), workItemHandlers = @WorkItemHandler(name = "MyWIH", value = BPMModelTests.TestWorkItemHandler.class))
public interface DoStuffProcess extends DoStuff {

    @Override
    @SignalEvent(eventId = "theEventId", globals = @Global(from = "context['foobar']", to = "globalVar"), inputs = @Input(from = "message.content.nested", to = "inputVar"), outputs = @Output(from = "outputVar", to = "message.content"), faults = @Fault(from = "faultVar", to = "message.content"))
