...
@Rules(channels = @Channel(name = "theName", operation = "theOperation", reference = "theReference", value = RulesModelTests.TestChannel.class), listeners = @Listener(DebugRuleRuntimeEventListener.class), loggers = @Logger(interval = 2000, log = "theLog", type = LoggerType.CONSOLE), manifest = @Manifest(
...
resources = {@Resource(location = "foo.drl", type = "DRL"), @Resource(location = "bar.dsl", type = "DSL"),
...
public interface DoStuffRules extends DoStuff {
    ...
    @Override
    @FireUntilHalt(eventId = "theEventId", globals = @Global(from = "context['foobar']", to = "globalVar"), inputs = @Input(from = "message.content.nested", to = "inputVar"), outputs = @Output(from = "outputVar", to = "message.content"), faults = @Fault(from = "faultVar", to = "message.content"))
